...
import { AlertCircle, Code2, Eye, EyeOff, Loader2 } from "lucide-react";
...
export default function LoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      login();
      // Internet Identity opens a popup — successful auth is handled by the
      // useInternetIdentity effect in useAuth. We listen for isAuthenticated
      // to navigate away, but since II uses a popup we just redirect after.
      navigate({ to: "/" });
    } catch {
      ...
      <div className="w-full max-w-md relative">
        ...
        <div className="glass-dark rounded-2xl p-8 shadow-xl">
          ...
          <p className="text-muted-foreground text-sm mb-6">
            Sign in with Internet Identity to save your code
          </p>
          ...
          <form onSubmit={handleSubmit} noValidate className="space-y-4">
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                className="w-4 h-4 rounded border-border bg-black/30 accent-primary cursor-pointer"
              />
              <label
          ...
          <div className="mt-3 text-center">
            <button
              type="button"
              data-ocid="login.guest_mode_button"
              onClick={() => navigate({ to: "/" })}
